...
|---|---|---|---|---|---|---|
| M1-T1 | Define n8n input contract for single file upload/path | G1 | High | none | Input schema documented and accepted by workflow entry node | todo |
| M1-T2 | Implement PDF validation (type/corruption/password checks) | G1 AC4-AC5 | High | M1-T1 | Invalid files are rejected with clear error payload/messages | todo |
| M1-T3 | Implement JA→RU command template using PDFMathTranslate-next | G3, Context constraints | High | M1-T1 | Exec node command finalized with `--lang-in ja --lang-out ru` and output directory | todo |
